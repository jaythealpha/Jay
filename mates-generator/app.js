...
const EMOJI_MAP = {
  dog: "🐶", dippoo: "🦛", "cat-calista": "🐱", "cat-cosmo": "🐱",
  bear: "🐻", moose: "🫎", monkey: "🐵", fox: "🦊",
  raccoon: "🦝", koala: "🐨", sloth: "🦥", hedgehog: "🦔",
  pig: "🐷", lion: "🦁", llama: "🦙", giraffe: "🦒",
  elephant: "🐘", panda: "🐼", owl: "🦉", penguin: "🐧",
  turtle: "🐢", "frog-francis": "🐸", "frog-ferdinand": "🐸",
  octopus: "🐙", shark: "🦈", dolphin: "🐬", yeti: "❄️",
  dragon: "🐲", trex: "🦖", dino: "🦕", unicorn: "🦄",
  alligator: "🐊", succulent: "🌵",
};
...
function renderCharacterGrid(filter = "") {
  ...
  state.characters
    ...
    .forEach((c) => {
      ...
      const kr = c.markets && c.markets.includes("KR");
      card.innerHTML = `
        ...
        <div class="char-meta">${c.species}${kr ? ' · <span class="kr">한국 판매</span>' : ""}</div>
      `;
...
function buildBaseDescription(character) {
  ...
  // palette 는 실물 확인이 끝난 캐릭터에만 들어간다 — 색을 지어내 넣으면
  // 실제 제품과 다른 색으로 그려지므로, 미확인이면 아예 빼고 레퍼런스 사진을 따르게 한다.
  const parts = [
    `Yogibo Mate plush character "${character.name_en}" (${character.species})`,
    `silhouette: ${character.silhouette}`,
  ];
  if (character.palette && character.palette_status === "verified") {
    parts.push(`color palette: ${character.palette}`);
  } else {
    parts.push("colours: match the official product photo exactly (do not invent colours)");
  }
  parts.push(`signature features: ${character.features}`);
...
function renderResults(blocks) {
  ...
  if (blocks.length && c && c.palette_status !== "verified") {
    const note = document.createElement("p");
    note.className = "ref-note";
    note.innerHTML = c.reference_360
      ? `🎨 <b>${escapeHtml(c.name_ko)}</b>의 색상은 아직 검증되지 않아 프롬프트에서 색을 지정하지 않았습니다. ` +
        `<a href="${escapeHtml(c.reference_360)}" target="_blank" rel="noopener">실물 제품컷 열기</a> 를 ` +
        `이미지 생성 도구에 함께 넣으면 실제 색으로 나옵니다.`
      : `🎨 <b>${escapeHtml(c.name_ko)}</b>는 한국 미판매라 실물 제품컷이 없어, 프롬프트에서 색을 지정하지 않았습니다. ` +
        `<a href="https://yogibo.jp/collections/mates" target="_blank" rel="noopener">yogibo.jp 제품 페이지</a> 의 ` +
        `사진을 참고 이미지로 넣어 주세요.`;
