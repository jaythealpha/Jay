...
- **푸시 토큰**: PushDevice.token은 민감 자격증명 — 로그·API 응답·Noop 발송
  기록 어디에도 값이 남지 않는다(카운트만). 무효 토큰은 발송 결과로 자동 삭제.
- **운영자 접근 분리**: 관리자 API는 사용자 JWT와 완전히 분리된 정적 운영
  토큰(타이밍 안전 비교), 기본 비활성(503). 응답은 카운트 전용 — 이메일·
  바코드·이벤트 메타데이터 미포함 (e2e로 직렬화 검증).
- 기기 OCR 원문(deviceOcrText)은 쿠폰 번호를 포함할 수 있어 파싱 직후 폐기.

## 마켓(M4)에서 추가된 방어

- **판매자 익명성**: 마켓 응답에 판매자 ID·이메일 미포함 (e2e 직렬화 검증).
- **바코드 보호 유지**: 판매 글에도 바코드 원문·해시 무노출 — 구매 완료 후
  새 소유자만 감사 로그를 남기며 열람 가능.
- **모의 결제 명시**: PaymentProvider Mock은 MOCK- 접두 paymentRef를 강제,
  UI에 "실제 돈이 오가지 않음" 배너 — 시뮬레이션이 실거래처럼 보이지 않게.
  카드 정보는 시스템 어디에도 존재하지 않는다.
- **동시성**: 소유권 이전은 LISTED 조건부 업데이트 트랜잭션 — 이중 판매 불가.
- 소유권 이전 시 이전 소유자의 만료 알림은 취소되고 새 소유자로 재예약.
